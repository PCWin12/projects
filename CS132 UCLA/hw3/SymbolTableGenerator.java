...
public class SymbolTableGenerator extends GJDepthFirst<SymbolType, SymbolType> {
	...
	public SymbolType visit(MainClass n, SymbolType argu) {
		...
		if (currenttable.symboltable.containsKey(n.f1.f0.toString())) {
			...
			mtd.args.put(n.f11.f0.toString(), new Array(n.f11.f0.toString()));
	 ...
	 * f8 -> ( Statement() )* f9 -> "return" f10 -> Expression() f11 -> ";"
	 * f12-> "}"
	 */
	...
	public String createHeader(SymbolTable table) {
		String header = "";
		for (Entry<String, Class> c : table.symboltable.entrySet()) {
			String name = c.getKey();
			Class cls = table.symboltable.get(name);
			cls.sizebytes = 4 + 4 * cls.getcomplete_vars(table.symboltable,
					cls.name).size();

			int j = 0;
			for (Map.Entry<String, SymbolType> f : cls.getcomplete_vars(
					table.symboltable, cls.name).entrySet()) {
				cls.vars_pos.put(f.getKey(), 4 + 4 * j);
				j++;
			}

			if (cls.funcs.containsKey("main")) {

			} else {
				String temp = "const vmt_" + cls.name;
				int i = 0;
				for (Map.Entry<String, Method> m : cls.getcomplete_funcs(
						table.symboltable, cls.name).entrySet()) {
					cls.funcs_pos.put(m.getKey(), 4 * i);
					temp = temp + "\n  " + ":" + cls.name+"."+m.getKey();
					;
					i++;
				}
